...
- The core innovation is context economy: only the actual listing loads at startup. In a
  ...
  Reference text enters only when read, and bundled scripts can execute without their
  source entering the context window.
...
filesystem protocol. Because a capable harness does not need to read every bundled file,
the unused bundle is effectively unbounded. In Claude Code,
`disable-model-invocation: true` removes a user-only Skill from the regular-session
startup listing until manual invocation. A custom subagent configured with `skills:` is
different: its named full Skill contents are injected at startup.
...
**Caveat:** In an eager-schema MCP host or configuration, up-front loading buys *runtime
latency*: the model reasons immediately about which tool to call, whereas a
filesystem-and-shell progressive-disclosure implementation needs retrieval round trips.
Independent analysis (MCPJam) notes progressive disclosure may use "3 to 4 times" as many
filesystem tool calls. The optimal design may be hybrid (e.g. Klavis Strata uses MCP as
delivery with progressive-disclosure-style discovery on top).
...
Anthropic's implementation runs Skills in a code execution environment where Claude has
filesystem access, Bash commands, and code execution capabilities. Its API copies Skills to
`/skills/{directory}/`, and Claude can use ordinary Bash to navigate. That is an
implementation shape, not a requirement of the portable Agent Skills format.
...
**API / Agent SDK:** via the code execution tool. Skills require code execution plus the
`skills-2025-10-02` beta header. The current quickstart uses generally available
`code_execution_20260521` with that Skills header alone; an older code-execution version
needs the header appropriate to that tool version. Add `files-api-2025-04-14` only when
the Files API uploads input to the container or downloads a Skill's output. Skills are
specified in `container` with `type` (`anthropic`/`custom`), `skill_id`, optional
`version`; up to **8 Skills/request**; uploads capped at **30 MB**. Custom Skills via
`/v1/skills` (CRUD + versioning); Anthropic Skills use date versions (`20251013`),
custom use epoch-timestamp or `latest`. Notes: changing the Skills list breaks prompt
caching; long-running Skills surface a `pause_turn` stop reason to feed back.
...
Context nuance: in a regular Claude Code session, an invoked Skill's content "enters the
conversation as a single message and stays there for the rest of the session" — a recurring
per-turn cost, not re-read each turn. An identical rendered re-invocation adds only a short
already-loaded note; a changed rendering, such as different arguments or dynamic-context
output, appends the full body again. Distinct rendered Skill bodies can coexist. A custom
subagent with a `skills:` list is the separate preloaded path: its named full Skill content
is injected at startup. Auto-compaction carries the most recent invocation of each Skill
forward within a 25,000-token budget.
...
your agent access to external tools and data. Skills teach your agent what to do with those
tools and data." MCP does not make that connection secure by itself. Its
[authorization specification](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization)
makes authorization optional, and its [security best practices](https://modelcontextprotocol.io/docs/tutorials/security/security_best_practices)
place authentication, transport, authorization, and least-privilege deployment on
implementations.
...
a sandbox alone cannot stop prompt injection or egress a system explicitly permits.
Microsoft found a (patched) Claude Code GitHub Action flaw where the Read tool bypassed the
Bubblewrap sandbox isolating Bash. Lesson: sandboxing, least-privilege permissions, explicit
