...
**The context-sharing tension** (connects to delegation's isolation theme). Two opposing pressures: *isolate* for cleanliness/parallelism (Anthropic's subagents each get their own window; the Claude Agent SDK spawns each subagent fresh, only the final message returns) vs *share* for coordination (without shared context, agents make conflicting implicit decisions — Cognition's Flappy Bird: "Subagent 1... started building a background that looks like Super Mario Bros. Subagent 2 built you a bird, but it doesn't look like a game asset," motivating Principle 1: "Share context, and share full agent traces, not just individual messages"). The narrower reconciliation is: **parallelize separable investigation and independently owned artifacts; serialize shared or coupled decisions and final integration.** Anthropic distributes *read/search* across isolated subagents and gives the lead synthesis, while its appendix permits specialized agents to persist independently owned code, reports, and visualizations and return lightweight references. [Cognition's current coding account](https://cognition.com/blog/multi-agents-working) supplies the different warning: parallel writers make conflicting implicit decisions, so its working pattern keeps writes single-threaded. Anthropic also uses an external filesystem as shared memory and persists the lead's plan to memory because context >200,000 tokens gets truncated.
...
**Is multi-agent worth it at all?** The June 2025 Cognition-vs-Anthropic debate is now a narrower distinction. Cognition's April 2026 follow-up says parallel writers fragment implicit decisions, while multiple agents can contribute intelligence when writes stay single-threaded. Anthropic (June 13): multi-agent beat single-agent by 90.2% for *research* (breadth-first, read-heavy, decomposable, exceeding one window), and "token usage by itself explains 80% of the variance" on BrowseComp. Synthesis (echoed by LangChain): **independent, high-value, breadth-first work can fan out; independently owned artifacts can persist under an explicit merge contract; shared or coupled decisions and final integration need one accountable owner.** Multi-agent earns its ~15× multiplier only when the task decomposes into independent, high-value, breadth-first strands. Coding, tightly-coupled reasoning, and anything needing shared write-state are poor fits — Anthropic concedes "most coding tasks involve fewer truly parallelizable tasks than research."
...
5. **Parallelize separable investigation and independently owned artifacts; serialize coupled decisions and final integration.** Distribute search/exploration across isolated subagents; allow a worker to persist an owned output only with a stable owner, location, and explicit merge contract; funnel shared decisions and final integration through one agent. Use external shared memory (files, blackboard, LangGraph state) for high-bandwidth handoff instead of lossy chat returns.
...
## Caveats
- The multi-agent value question is genuinely contested — the Cognition-vs-Anthropic split is real and unresolved; both positions are correct for their task class (coupled write-state versus independently decomposable research and owned artifacts). Don't treat "multi-agent = better" as settled.
