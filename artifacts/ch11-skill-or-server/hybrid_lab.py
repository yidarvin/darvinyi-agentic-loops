...
folder release_notes_skill/, whose bundled formatter the lab executes.
"""
...
FORMATTER = os.path.join(HERE, "release_notes_skill", "scripts", "format_notes.py")
...
def classify(access: bool, judgment: bool, shared: bool,
    ...
    server_needed = shared or live or (access and not cli_or_existing)

    if server_needed and judgment:
        reasons = [
    ...
    if server_needed and not judgment:
        reasons = [
    ...
    if access and cli_or_existing:
        reasons = [
            "Access is needed, but a CLI or an existing server already provides it. "
            "Wrap that in a skill before building a server of your own.",
        ]
        if judgment:
...
def run_tests() -> int:
    ...
    try:
        ...
        srv.close()
    check("server returns commits for the fixture range", len(result["commits"]) >= 1)
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--tag", default="v0.3.0", help="fetch commits merged since this tag")
